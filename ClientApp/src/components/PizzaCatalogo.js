...
export class PizzaCatalogo extends Component {
    ...
    create = (item) => {
        ...
        authService.getAccessToken().then((token) => {
            const opcines = { headers: !token ? {} : { 'Authorization': `Bearer ${token}` } }
        })
    ...
    render() {
        return ( <
            ...
            Table > <
            /Container> <
            Modal isOpen = { this.state.accion > 0 && true }
            centered toggle = { this.mitoogle }
            ...
            onChange = { this.handleChange } > {
                this.state.salsas.map(
                    salsa =>
                    <
                    option value = { salsa.id }
                    selected = { this.state.accion === 2 && this.state.pizzaE.sauce.id === salsa.id } > { salsa.name } < /option>
                )
            } <
            /Input> < /
            FormGroup > <
            ...
            onChange = { this.handleChange } > {
                this.state.ingredientes.map(
                    ...
                    selected = { this.state.accion === 2 && this.marcarIngre(topping) } > { topping.name } < /option>
                )
            } <
            /Input> < /
            FormGroup > <
